...
//                    System.out.println(bddEngine.satCount(entry.getValue()) + " " + bddSat);
    ...
    private static void printRec(NDD current) {
        ...
        // 新增：收集所有 BDD 节点（按 field 分组）和根节点集合
    private static void collectBDDNodes(NDD ndd, Set<NDD> visitedNDD,
                                        Map<Integer, Set<Integer>> fieldToBDDNodes,
                                        Set<Integer> rootBDDs) {
        if (ndd.isTerminal() || visitedNDD.contains(ndd)) return;
        visitedNDD.add(ndd);

        for (Map.Entry<NDD, Integer> entry : ndd.getEdges().entrySet()) {
            int bddId = entry.getValue();
            int field = ndd.getField();
            rootBDDs.add(bddId);
            fieldToBDDNodes.computeIfAbsent(field, k -> new HashSet<>())
                           .addAll(collectBDDTree(bddId, new HashSet<>()));
            collectBDDNodes(entry.getKey(), visitedNDD, fieldToBDDNodes, rootBDDs);
        }
    }

    // 新增：递归收集一个 BDD 根节点的所有子节点（包括自身）
    private static Set<Integer> collectBDDTree(int bddId, Set<Integer> visited) {
        if (bddId <= 1 || visited.contains(bddId)) return visited;
        visited.add(bddId);
        int high = bddEngine.getHigh(bddId);
        int low = bddEngine.getLow(bddId);
        collectBDDTree(high, visited);
        collectBDDTree(low, visited);
        return visited;
    }
    ...
    public static void printDot(NDD root, String filename) {
        ...
        // 收集所有 BDD 节点（按 field 分组）和根节点集合
        Map<Integer, Set<Integer>> fieldToBDDNodes = new HashMap<>();
        Set<Integer> rootBDDs = new HashSet<>();
        Set<NDD> visitedNDD = new HashSet<>();
        collectBDDNodes(root, visitedNDD, fieldToBDDNodes, rootBDDs);

        // 为每个 field 绘制 BDD 子图
        for (Map.Entry<Integer, Set<Integer>> entry : fieldToBDDNodes.entrySet()) {
            int field = entry.getKey();
            Set<Integer> bddNodes = entry.getValue();

            sb.append("  subgraph cluster_field_").append(field).append(" {\n");
            sb.append("    label=\"Field ").append(field).append("\";\n");
            ...
            // 绘制所有非终端 BDD 节点
            for (int bddId : bddNodes) {
                if (bddId <= 1) continue;
                int var = bddEngine.getVar(bddId);
                String label;
                if (rootBDDs.contains(bddId)) {
                    label = "#" + bddId + "\\nx" + var;   // 根节点显示 ID 和变量
                } else {
                    label = "x" + var;                    // 内部节点只显示变量
                }
                sb.append("    bdd_").append(bddId)
                  .append(" [shape=circle, label=\"").append(label).append("\"];\n");
            }
            ...
            for (int bddId : bddNodes) {
                ...
                // 高边
                if (high != 0) {
                    String highTarget = (high == 1) ? "TRUE_" + field : "bdd_" + high;
                    sb.append("    bdd_").append(bddId).append(" -> ").append(highTarget)
                      .append(" [style=solid];\n");
                }

                // 低边
                if (low != 0) {
                    String lowTarget = (low == 1) ? "TRUE_" + field : "bdd_" + low;
                    sb.append("    bdd_").append(bddId).append(" -> ").append(lowTarget)
                      .append(" [style=dotted];\n");
                }
            }
        ...
        // 绘制 NDD 结构
        Set<NDD> visitedNDDForPrint = new HashSet<>();
        printNDDStructure(root, sb, visitedNDDForPrint);
    ...
    private static void printNDDStructure(NDD current, StringBuilder sb,
        ...
        for (Map.Entry<NDD, Integer> entry : current.getEdges().entrySet()) {
            ...
            sb.append("  ").append(nodeId).append(" -> ").append(nextId)
              .append(" [label=\"bdd_").append(bddId)
