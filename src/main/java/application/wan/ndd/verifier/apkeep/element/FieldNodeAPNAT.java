...
import application.wan.ndd.verifier.apkeep.core.BDDRuleItem;
import application.wan.ndd.verifier.apkeep.core.ChangeItem;
...
public class FieldNodeAPNAT extends FieldNodeAP {
    ...
    public ArrayList<ChangeItem> InsertRewriteRule(RewriteRule rule) {
        if (!ports.contains(rule.name)) {
            ...
            // System.out.println(name+" "+rule.name+" "+NDD.toBDD(rule.new_src)+"
            // "+NDD.toBDD(rule.new_dst));
