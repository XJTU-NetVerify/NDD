...
import application.wan.ndd.verifier.common.Utility;
...
public class NetworkNDDAPNAT extends NetworkNDDAP {
    ...
    // int nddPred1 =
    // bdd_engine.getBDD().ref(NDD.toBDD(d.ports_action_pred.get(port).getKey()));
    // int moleculePred1 =
    // bdd_engine.getBDD().ref(Molecule.toBDD(d.ports_action_aps.get(port).getKey()));
    // int nddPred2 =
    // bdd_engine.getBDD().ref(NDD.toBDD(d.ports_action_pred.get(port).getValue()));
    // int moleculePred2 =
    // bdd_engine.getBDD().ref(Molecule.toBDD(d.ports_action_aps.get(port).getValue()));
    // if(nddPred1 != moleculePred1 || nddPred2 != moleculePred2)
    // {
    // System.out.println(d.name+" "+port+" "+nddPred1+" "+moleculePred1+"
    // "+nddPred2+" "+moleculePred2);
    // }
    ...
    public void updateNATRule(ArrayList<String> NAT_rules) {
        for (String rule : NAT_rules) {
            ...
            if (tokens[7].equals("any")) {
                ...
                srcBDD = NDD.encodePrefixBDD(ipbin, NDD.getBDDVars(bdd_engine.SRC_IP_FIELD),
                        NDD.getNotBDDVars(bdd_engine.SRC_IP_FIELD));
            }
            ...
            if (tokens[8].equals("any")) {
                ...
                dstBDD = NDD.encodePrefixBDD(ipbin, NDD.getBDDVars(bdd_engine.DST_IP_FIELD),
                        NDD.getNotBDDVars(bdd_engine.DST_IP_FIELD));
            }
    ...
    public void UpdateFieldAP() {
        ...
        for (FieldNodeAP device : FieldNodes.values()) {
            ...
            if (device.type == 2) {
                HashSet<Pair<AtomizedNDD, AtomizedNDD>> oldSetAction = new HashSet<>(
                        ((FieldNodeAPNAT) device).ports_action_aps.values());
                ...
                for (Pair<String, String> p : ports) {
                    if (!finished.contains(p)) {
                        ...
                        if (field == 0) {
                            ...
                            AtomizedNDD new_aps = AtomizedNDD.splitSingleFieldAtomsWithSingleFieldPredicate(split_ap,
                                    aps);
                            if (!new_aps.isFalse()) {
                                AtomizedNDD.ref(new_aps);
                                AtomizedNDD.deref(aps);
                ...
                for (Pair<String, String> p : ports) {
                    if (!finished.contains(p)) {
                        ...
                        if (field == 0) {
                            AtomizedNDD aps = pair.getKey();
                            Pair<Boolean, AtomizedNDD> ret = AtomizedNDD
                                    .splitMultipleFieldsAtomsWithMultipleFieldsPredicate(split_ap, aps);
                            if (ret.getKey()) {
                                AtomizedNDD.ref(ret.getValue());
                                AtomizedNDD.deref(aps);
